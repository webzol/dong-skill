...
> 📦 本目录已并入 **gzh-skill**（仓库 `webzol/dong-skill`）的 **design 引擎**。统一入口与路由规则见上级目录 `gzh-skill/SKILL.md`；安装方式以 `gzh-skill/README.md` 为准。
...
读 [references/theme-index.md](references/theme-index.md)（主题信息的单一来源）。**用户没指定主题时，把全部主题列给用户选，不替用户定**：

- **用户已在请求里指定主题** → 直接用，不问。
- **用户说"让我选风格"** → 读取 theme-index.md 全部主题，列出中文名 + 适用场景供选择。可提示用户打开 `references/theme-preview.html` 查看可视化预览。
- **题材有明确契合** → 可标"（推荐）"放第一位，但必须让用户确认。
- **无明显倾向** → 列出全部主题供选择。
...
- 内刊手记 / 深度评测 / 案例复盘 → 橄榄手记
- 深度观点 / 专题报道 / 国际编辑部气质 → 墨蓝刊读
- AI / 科技 / 产品发布 / 数据驱动型文章（明亮现代）→ 科技橙
- AI / 科技 / 产品发布 / 数据驱动型文章（低调沉稳）→ 科技黑
...
**输出目录约定（每次必做）**：在当前工作目录下新建（或复用）**当天日期目录** `generated/<YYYY-MM-DD>/`（本地日期，ISO 格式），本次排版的所有产物只放这个目录，**最终只交付一个文件**：

- **干净正文（中间产物）**：先写入 `generated/<YYYY-MM-DD>/` 下。
- **带「复制」按钮的成品（唯一交付物）**：校验通过后用 `wrap_preview.py <干净正文.html> <成品文件名.html>` 生成，成品 = 干净正文包一层预览外壳。
- **删除中间产物**：成品生成后删掉干净正文文件，目录里只留带按钮的成品一个文件。
- 告知用户：**打开成品 → 点右上角「复制」→ 公众号编辑器粘贴**；附校验脚本结论。
