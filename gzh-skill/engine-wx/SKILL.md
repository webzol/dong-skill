...
**参数（均可缺省，缺省用默认「简约科技 × DONGPro极简科技」）：**

- `--template` 模板名：`minimal-tech`（简约科技·默认）/ `cyber-card`（卡片）/ `terminal-code`（终端风）/ `hud`（数据面板）/ `wechat-pai-essay`（微信派·文艺长文）
- `--theme` 主题名：`dongpro`（DONGPro极简科技·默认）/ `minimal-light`（极简白）/ `minimal-dark`（极简黑）/ `tech-blue`（科技蓝）/ `cyber-green`（赛博绿）/ `neon-purple`（霓虹紫）/ `taobao-flash`（淘宝闪购）/ `wechat-pai`（微信派）
- `[草稿]` 文章内容：文件路径，或直接内联
...
1. 用 `Start-Process` 在浏览器中打开 `preview/selection.html`（相对于本 skill 目录的路径）。该页面是一个类软件设置面板风格的深色交互页：
   ...
   - 中侧「主题色调」面板：8 个主题选项，每个带色块预览 + 颜色值，点击切换激活主题；`DONGPro极简科技` 标有金色「推荐」标签
   - 右侧实时预览区：根据当前激活的模板 × 主题渲染文章样例，尺寸 677px 宽，缩放 42% 居中显示，**点击可放大全屏查看**，ESC 或点击背景关闭；底部有「点击放大查看」提示
   - 默认组合：`简约科技 × DONGPro极简科技`
   - 底部「★ 推荐」按钮一键切回默认组合并弹出确认框
...
| 主题 | 风格 | 关键色 |
|---|---|---|
| `dongpro`·**默认** | DONGPro极简科技 | 白底 + #141414 近黑 + 苹果字体栈 + 1.95 行高 + 紫蓝 accent `#615ced` |
