...
**参数（均可缺省，缺省用默认「简约科技 × DONGPro极简科技（排版推荐）」）：**

- `--template` 模板名：`minimal-tech`（简约科技·默认）/ `cyber-card`（卡片）/ `terminal-code`（终端风）/ `hud`（数据面板）/ `wechat-pai-essay`（微信派·文艺长文）
- `--theme` 主题名：`dongpro`（DONGPro极简科技 · **排版推荐**·默认）/ `minimal-light`（极简白）/ `minimal-dark`（极简黑）/ `tech-blue`（科技蓝）/ `cyber-green`（赛博绿）/ `neon-purple`（霓虹紫）/ `taobao-flash`（淘宝闪购）/ `wechat-pai`（微信派）/ `qianwen`（千问）
- `[草稿]` 文章内容：文件路径，或直接内联
...
### 第 1.5 步　打开模板 × 主题选择页（首次排版时必做）

**触发条件**：用户没有明确指定 `--template` 和 `--theme`（或只指定了其一）。

1. 用 `Start-Process` 在浏览器中打开 `preview/selection.html`（相对于本 skill 目录的路径）。该页面是一个类软件设置面板风格的深色交互页：
   - 左侧「模板」面板：5 个模板选项，每个带 SVG 缩略图 + 名称 + 描述，点击切换激活模板；`简约科技` 标有「默认」标签
   - 中侧「主题色调」面板：9 个主题选项，每个带色块预览 + 颜色值，点击切换激活主题；`DONGPro极简科技` 标有金色「排版推荐」标签
   - 右侧实时预览区：根据当前激活的模板 × 主题渲染文章样例，尺寸 677px 宽，缩放 42% 居中显示，**点击可放大全屏查看**，ESC 或点击背景关闭；底部有「点击放大查看」提示
   - 默认组合：`简约科技 × DONGPro极简科技（排版推荐）`
   - 底部「★ 排版推荐」按钮一键切回默认组合并弹出确认框
   - 底部「确认使用此组合」按钮弹出确认模态框（含组合信息 + 一键复制按钮）

2. 提示用户：
   > 已打开排版组合选择页。**推荐组合（简约科技 × DONGPro极简科技）已默认激活**，右侧实时预览区可见效果。可点击左侧模板或中侧主题切换，确认后底部按钮「确认使用此组合」弹出组合代码，复制后发回给 AI 即可完成排版。

3. **等待用户回复**（最多 60 秒），收到回复后再继续第 2 步。
   - 若用户回复"用推荐的"、「确认」或直接不回复（超时）→ 使用默认：`minimal-tech × dongpro`
   - 若用户回复"用 cyber-card 赛博绿" → 解析为 template=`cyber-card`，theme=`cyber-green`
   - 若用户回复"全部看一下" → 不自动选，等用户从选择页中选定并告诉 AI 结果

4. 若用户已明确指定模板/主题（如"用 terminal-code 赛博绿"），则跳过此步直接进入第 2 步。

### 第 2 步　锁定模板 + 主题
- 读 `templates/_index.md` 与 `themes/themes.json`，看清单与各自风格一句话说明。
- 模板决定**版式骨架**（怎么摆块），主题决定**颜色**（色板）。两者自由组合。
- 默认 `minimal-tech` + `dongpro`。除非用户点名，否则别自作主张换更强效果的组合。
...
| 主题 | 风格 | 关键色 |
|---|---|---|
| `dongpro`·**排版推荐** | DONGPro极简科技 | 白底 + #141414 近黑 + 苹果字体栈 + 1.95 行高 + 紫蓝 accent `#615ced` |
| `minimal-light` | 极简白 | 白底、灰字、单一黑/蓝 accent |
